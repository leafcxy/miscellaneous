...
$(function () {
    ...
    if (-1 == WebVideoCtrl.I_CheckPluginInstall()) {
        if (window.confirm('请下载视频插件')) {
            location.href = "/Assets/js/videos/codebase/WebComponents.exe";
        }
        //else {
        //    alert("取消下载");
        //}
    ...
    // 初始化插件参数及插入插件
    WebVideoCtrl.I_InitPlugin($("#divPlugin").width(), $("#divPlugin").height(), {
        iWndowType: 1,
...
// 登录  登录IP   登录端口  登录用户  登录密码  通道号
function clickLogin(IP,Port,User,Password,PortNum) {
    var szIP = IP,//$("#loginip").val(),
		szPort = Port,//$("#port").val(),
		szUsername = User,//$("#username").val(),
		szPassword = Password;//$("#password").val();
    ...
    var iRet = WebVideoCtrl.I_Login(szIP, 1, szPort, szUsername, szPassword, {
        success: function (xmlDoc) {
            ...
            clickStartRealPlay(szIP, PortNum);
...
// 开始预览
function clickStartRealPlay(IP,PortNum) {
    var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
		szIP = IP,//$("#ip").val(),
		iStreamType = parseInt(1, 10),//parseInt($("#streamtype").val(), 10),
		iChannelID = PortNum,//parseInt($("#channels").val(), 10),
...
// 抓图
function clickCapturePic(channels) {
    var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
		szInfo = "";

    if (oWndInfo != null) {
        var szChannelID = channels,//$("#channels").val(),
