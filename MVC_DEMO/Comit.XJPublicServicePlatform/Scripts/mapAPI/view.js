...
(function () {
    var flagsPath = 'http://api.shipxy.com/apiresource/flags/', //国旗图标服务器路径
        ...
        customWins = [], //自定义框
        filterWinWidth = 480,
        hideEventName = null;//隐藏事件函数

    var country_zh = {
        'ADL': '阿黛利地', 'ALB': '阿尔巴尼亚', 'DZA': '阿尔及利亚', 'AFG': '阿富汗', 'ARG': '阿根廷',
    ...
    //航迹回放页面
    movieConfigView = "<iframe id='movieInfo' name='movieInfo' frameborder='0' src='../../Common/MapOCX/MovieAction?$@params@$'  scrolling='yes'  style='width:100%;height:480px;'></iframe>"
        + "<br>"
        + " <div class='col-xs-12 col-md-6 col-lg-4 div-padding' id='elapsedDiv' style='display:none'>"
        + "    <span>"
        + "        当前回放时间："
        + "    </span><span id='elapsedbackup'>等待回放中……</span>,&nbsp;&nbsp;&nbsp;&nbsp;"
        + " <span>"
        + "    当前回放倍速："
        + "  </span><span id='speedCountbackup'>0</span>倍。"
        + "</div>"
        + "<div><br>"
        + "    <div id='progress'>"
        + "        "
        ...
        + "        "
        + "    </div>"
        + "    <div class='btnLink' id='btnLinkDiv' style='text-align:left'>"
        + "    <a href='javascript:void(0)' id='operator' >播放</a>"
        + "    <a href='javascript:void(0)' id='stop' >暂停</a>"
        + "    <a href='javascript:void(0)' id='fastForward' >快进</a>"
        + "    <a href='javascript:void(0)' id='fastBack' >快退</a>"
        + "    <a href='javascript:void(0)' id='addSpeed' >加速</a>"
        + "    <a href='javascript:void(0)' id='reduceSpeed' >减速</a></div>"
        + "</div>";
    //轨迹查询页面
    trackConfigView = "<iframe id='trackInfo' frameborder='0' src='../../Common/MapOCX/TrackAction?$@params@$'  scrolling='no' style='width:100%;height:440px;'></iframe>"
        + ""
        + "<div>"
        + "    <div class='btnLink' id='btnLinkDiv'>"
        + "    <a href='javascript:void(0)' id='search' >查询</a>"
        + "    <a href='javascript:void(0)' id='cancle' onclick='hideContent(\"取消\",this)' >取消</a>"
        + " </div>"
    + "</div>";
    //历史位置查询页面
    historyConfigView = "<iframe id='historyInfo' frameborder='0' src='../../Common/MapOCX/HistoryPositionAction'  scrolling='no' style='width:100%;height:380px;'></iframe>"
        + "<div>"
        + "    <div class='btnLink' id='btnLinkDiv' >"
        + "    <a href='javascript:void(0)' id='historyPosition' width='100px'>查询历史</a>"
        + "    <a href='javascript:void(0)' id='currentPosition' width='100px' >定位当前</a></div>"
        + "</div>";


    var historyEvent = function () {
        //历史定位
        myApp.util.addEvent(myApp.util.getElement("historyPosition"), "click", function (event) {
            myApp.util.stopPropagation(event);
            if ($("#historyInfo").contents().find("#historyTime").val() == "") {
                myApp.Message.alert('请输入查询时间！');
                return;
            }
            if ($("#historyInfo").contents().find("#selShipId").val() == "") {
                myApp.Message.alert("请选择查询船舶！");
                return;
            }
            myApp.service.showShipsHistory($("#historyInfo").contents().find("#selShipId").val(), $("#historyInfo").contents().find("#historyTime").val(), 'shipHistory');
            myApp.util.preventDefault(event);
        });
        //定位当前
        myApp.util.addEvent(myApp.util.getElement("currentPosition"), "click", function (event) {
            myApp.util.stopPropagation(event);
            //先回到实时动态页面
            myApp.service.stopShipsHistory();
            //myApp.util.formatTime(new Date().getTime().toString().substring(0, 10))
            //$("#historyInfo").contents().find("#historyTime").val();
            //if ($("#historyInfo").contents().find("#historyTime").val() == "") {
                myApp.service.showShipsHistory($("#historyInfo").contents().find("#selShipId").val(), null, 'shipHistory');
            //}
            $("#historyInfo").contents().find("#historyTime").val('');
            $("#historyInfo").contents().find("#historyTime").val("");
            if ($("#historyInfo").contents().find("#selShipId").val() == "") {
            ...
            //myApp.service.showShipsHistory(historyInfo.selShipId.value, $("#historyInfo").contents().find("#historyTime").val(), 'shipHistory');
            //执行定位
            setTimeout(function () {
                if ($("#historyInfo").contents().find("#selShipId").val().split(',').length == 1) {
                    myApp.service.locateShip($("#historyInfo").contents().find("#selShipId").val());
                }
                else {
                    myApp.service.locateShip($("#historyInfo").contents().find("#selShipId").val().split(',')[0]);
                }
                //停止动态，防止定位失焦
                myApp.service.startShipsHistory();
            }, 1000);
            myApp.util.preventDefault(event);
        });
    }
    //注册关闭事件
    hideTrackEvent = function () {
        myApp.map.removeOverlayByType(shipxyMap.OverlayType.SHIP); //删除所有船舶
        myApp.map.removeOverlayByType(shipxyMap.OverlayType.TRACK); //删除所有轨迹  
        setTimeout("myApp.service.stopShipsHistory()", 1000);
    };
    //注册按钮事件
    var trackEvent = function () {
        myApp.util.addEvent(myApp.util.getElement("search"), "click", function (event) {
            myApp.util.stopPropagation(event);
            if ($("#trackInfo").contents().find("#startTime").val() == "") {
                myApp.Message.alert('请输入开始时间！');
                return;
            }

            if ($("#trackInfo").contents().find("#endTime").val() == "") {
                myApp.Message.alert("请输入结束时间！");
                return;
            }
            if ($("#trackInfo").contents().find("#startTime").val() > $("#trackInfo").contents().find("#endTime").val()) {
                myApp.Message.alert("结束时间不能小于开始时间！");
                return;
            }
            if ($("#trackInfo").contents().find("#startTime").val() > myApp.util.formatTime(new Date().getTime().toString().substring(0, 10)) || $("#trackInfo").contents().find("#endTime").val() > myApp.util.formatTime(new Date().getTime().toString().substring(0, 10))) {
                myApp.Message.alert("查询时间不能大于当前时间！");
                return;
            }
            if ($("#trackInfo").contents().find("#selShipId").val().Trim() == "") {
                myApp.Message.alert("请选择查询船舶！");
                return;
            }
            myApp.service.searchTrack($("#trackInfo").contents().find("#selShipId").val(), $("#trackInfo").contents().find("#startTime").val(), $("#trackInfo").contents().find("#endTime").val());
            myApp.util.preventDefault(event);
        });
        ...
        //暂停事件
        myApp.util.addEvent(myApp.util.getElement("cancle"), "click", function (event) {
            myApp.util.stopPropagation(event);
            myApp.map.removeOverlayByType(shipxyMap.OverlayType.SHIP); //删除所有船舶
            myApp.map.removeOverlayByType(shipxyMap.OverlayType.TRACK); //删除所有轨迹  
            myApp.util.preventDefault(event);
    ...
    //注册回放事件
    hideMovieEvent = function () {
        $("#movieInfo").contents().find("#startTime").attr("disabled", null);
        $("#movieInfo").contents().find("#endTime").attr("disabled", null);
        $("#movieInfo").contents().find("#ShipId_chosen").attr("disabled", null);
        $("#movieInfo").contents().find("#selectAll").attr("disabled", null);
        $("#movieInfo").contents().find("#ShipId_chosen").val("");
        $("#movieInfo").contents().find("#elapsed").html("等待回放中……");
        $("#elapsedbackup").html( $("#movieInfo").contents().find("#elapsed").html());
        $("#movieInfo").contents().find("#startTime").val("");
        $("#movieInfo").contents().find("#endTime").val("");
        $("#progress").anim_progressbar.playTime = 0;
        $("#progress").anim_progressbar.intervalCount = 1;
        $("#progress").anim_progressbar.init("progress");
        setTimeout("myApp.service.stopShipsHistory()", 1000);
    };
    //注册航迹回放事件
    var movieEvent = function () {
        ...
        myApp.util.addEvent(myApp.util.getElement("operator"), "click", function (event) {
            myApp.util.stopPropagation(event);
            ...
            if ($("#movieInfo").contents().find("#endTime").val() == "") {
                myApp.Message.alert("请输入结束时间！");
                return;
            }
            if ($("#movieInfo").contents().find("#startTime").val() > $("#movieInfo").contents().find("#endTime").val()) {
                myApp.Message.alert("结束时间不能小于开始时间！");
                return;
            } 
            if ($("#movieInfo").contents().find("#startTime").val() > myApp.util.formatTime(new Date().getTime().toString().substring(0, 10)) || $("#movieInfo").contents().find("#endTime").val() > myApp.util.formatTime(new Date().getTime().toString().substring(0, 10))) {
                myApp.Message.alert("播放时间不能大于当前时间！");
                return;
            }
            if ($("#movieInfo").contents().find("#selShipId").val() == "" && $("#movieInfo").contents().find("#selectAll").attr("checked") != "checked") {
                myApp.Message.alert("请选择回放船舶！");
                return;
            }
            if ($("#movieInfo").contents().find("#startTime").attr("disabled") == null) {
                $("#movieInfo").contents().find("#startTime").attr("disabled", "disabled");
                $("#movieInfo").contents().find("#endTime").attr("disabled", "disabled");
                $("#movieInfo").contents().find("#ShipId_chosen").attr("disabled", "disabled");
                $("#movieInfo").contents().find("#selectAll").attr("disabled", "disabled");
                $("#progress").anim_progressbar({ start: $("#movieInfo").contents().find("#startTime").val(), finish: $("#movieInfo").contents().find("#endTime").val(), interval: 100 });
            }
            $("#progress").anim_progressbar.stopFlag = false;

            myApp.util.preventDefault(event);
        });
        //暂停事件
        myApp.util.addEvent(myApp.util.getElement("stop"), "click", function (event) {
            myApp.util.stopPropagation(event);
            $("#progress").anim_progressbar.stopFlag = true;
            myApp.util.preventDefault(event);
        });
        //加速事件
        myApp.util.addEvent(myApp.util.getElement("addSpeed"), "click", function (event) {
            myApp.util.stopPropagation(event);
            $("#progress").anim_progressbar.intervalCount = $("#progress").anim_progressbar.intervalCount * 3;
            myApp.util.preventDefault(event);
        });
        //减速事件
        myApp.util.addEvent(myApp.util.getElement("reduceSpeed"), "click", function (event) {
            myApp.util.stopPropagation(event);
            if ($("#progress").anim_progressbar.intervalCount > 1) {
                $("#progress").anim_progressbar.intervalCount = $("#progress").anim_progressbar.intervalCount / 3;
            }
            myApp.util.preventDefault(event);
        });
        //快进事件
        myApp.util.addEvent(myApp.util.getElement("fastForward"), "click", function (event) {
            myApp.util.stopPropagation(event);
            $("#progress").anim_progressbar.playTime += 5 * 60 * 1000;
            myApp.util.preventDefault(event);
        });
        //快退事件
        myApp.util.addEvent(myApp.util.getElement("fastBack"), "click", function (event) {
            myApp.util.stopPropagation(event);
            if ($("#progress").anim_progressbar.playTime > 5 * 60 * 1000) {
                $("#progress").anim_progressbar.playTime -= 5 * 60 * 1000;
            }
            else {
                $("#progress").anim_progressbar.playTime = 0;
    ...
    var createShipWinDataHtml = function (data) {
        ...
        return '<table id="infoTable" class="xinfo"><tr>'
            ...
            + '</tr><tr>'
            + '<td>MMSI：</td><td><div class="o" id="MMSI" title="' + data.MMSI + '">' + data.MMSI + '</div></td>'
            ...
            + '</tr><tr>'
            + '<td>船籍：</td><td><div class="o" title="' + (country_zh[data.country] || '中国') + '">' + (country_zh[data.country] || '中国') + '</div></td>'
    ...
    var addShipWinBtnEvent = function () {
        ...
        util.addEvent(util.getElement('btnTrack', parent), 'click', function (event) {
            util.stopPropagation(event); 
            if (util.getElement("trackInfo") != null) {
                util.getElement("trackInfo").src = util.getElement("trackInfo").src.toString().split('?')[0] + '?shipIds=' + util.getElement("MMSI", parent).innerHTML;
            }
            myApp.view.showCustomWins('历史轨迹查询', trackConfigView, 'trackEvent()', hideTrackEvent, 'shipIds=' + util.getElement("MMSI", parent).innerHTML);
    ...
    myApp.view = {
        ...
        showShipWin: function (data) {
            ...
            $.ajax({
                ...
                data: { shipIds: data.shipId, historyTime: myApp.util.formatTime(data.lastTime)},
                dataType: "json",
                async: false,
                beforeSend: function (shipData) {
                },
                success: function (shipData) {
                    if (shipData.length != 0) {
                        data = service.convertToApiDate(shipData)[0];
        ...
        //显示所有船舶
        showAllShip: function(e)
        {
            //显示全部船舶之前，显示出来监控船舶，要不然点轨迹查询它把所有船隐藏了的情况下会报错
            if (e.text() == "全部船舶")
            {
                if_supervision = 0;
            }
        ...
        //显示自定义框（带最小化）
        showCustomWinsWithMini: function (title, customContent, customEvent, hideEvent, params) {
            //给页面传参
            if (params != "") {
                customContent = customContent.replace("$@params@$", params);
            }
            else {
                customContent = customContent.replace("?$@params@$", "");
            }
            //判断当前框页面上是否已存在，存在则不重新生成
            for (var i = 0; i < customWins.length; i++)
            {
                if (customWins[i].title.indexOf(title)!=-1)
                {                    
                    if (customEvent != "") {//如果有事件的窗口，则重新显示旧的
                        if (title == "航迹回放")
                        {
                            service.stopShipsHistory();
                            customWins[i].hide();
                            setTimeout('$("#movieInfo").attr("src", "../../Common/MapOCX/MovieAction?'+ params+'" )',100);
                        }
                        else if (title == "历史轨迹查询")
                        {
                            service.stopShipsHistory();
                            customWins[i].hide();
                            setTimeout('$("#movieInfo").attr("src", "../../Common/MapOCX/TrackAction?' + params + '" )', 100);
                        }
                        customWins[i].show();
                        return;
                    }
                    else {//如果无事件的窗口则，把标题设置为删除，以便重新生成窗口
                        //customWins[i].title = "delete";
                        customWins[i].show();
                        return;
                    }
                }
            }
            hideEventName = hideEvent;
            var customWin = new myApp.XWin({
                modal: false,
                resizable: false,
                dragbody: true,
                width: filterWinWidth - 20,
                position: [document.body.clientWidth - shipWinWidth - 60, 200], //[left,top]
                title: title + "<a href='javascript:void(0)' onclick='hideContent(\"" + title + "\",this)' class='xwin_close1' id='xwin_close1'>[最小化]</a>",
                content: customContent, //自定义框内容
                onhide: function () {
                    if (hideEventName != "" && hideEventName != null) {
                        hideEventName();
                    }
                }
            });
            eval(customEvent); //注册自定义按钮的点击事件
            customWin.show(); //显示
            customWins.push(customWin);

        },
        //显示自定义框（没有最小化）
        showCustomWins: function (title, customContent, customEvent, hideEvent, params) {
            //给页面传参
            if (params != "") {
                customContent = customContent.replace("$@params@$", params);
            }
