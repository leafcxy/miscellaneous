...
if ($isIE) {
    ...
    if (!navigator.plugins["Supcan Plugin"])
    {
        var a = $("<a href='http://www.supcan.cn/install_chrome.htm' target='_blank'></a>").get(0);    
...
//////////////控件不能放在form标签里面,放在form里面则得写成form1.AF1.func();//////////////

//public:
function insertTreeList(id, para)	{ document.write( bldStr("BCV1.TreeList",id, para) ) }
function insertTreeList2(id, para)	{ return bldStr("BCV1.TreeList",id, para) }
...
function insertReport2(id, para) { return bldStr("LuxForm", id, para) }
...
function insertUpload(id, para)		{ return bldStr("BCV1.Upload",  id, para) }
